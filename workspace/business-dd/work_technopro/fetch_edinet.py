# -*- coding: utf-8 -*-
"""テクノプロ・ホールディングス (6028) の有報をEDINET API v2で5期分取得する。6月決算 → 9月提出。"""
import os
import time
import json
import requests
...
SEC_CODE = "60280"  # 6028 + trailing 0
OUT_DIR = Path(r"c:\Users\Kamei.Kenshi\Documents\dev\claude-code-book-template\workspace\business-dd\work_technopro\edinet")
...
def find_yuho(year):
    """6月決算 → 9月後半提出。広めに9/1-10/15を走査。"""
    for month, day_start, day_end in [(9, 1, 30), (10, 1, 15)]:
...
def main():
    # FY2021(=Jun2021,提出2021/9) 〜 FY2025(=Jun2025,提出2025/9) の5期
    targets = [2021, 2022, 2023, 2024, 2025]
    summary = []
    for y in targets:
        print(f"=== yuho filed in {y} (FY end Jun/{y}) ===")
