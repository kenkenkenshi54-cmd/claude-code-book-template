...
**自動モード＝既定（プロンプト最小化方針）**。他スキルからの委譲時はもちろん、
**ユーザーが直接 media-search を使った場合もこれを既定**とする。
ユーザー確認を**スキップ**し、Step 1 で明らかに無関係なものを除いた候補を**全件確定**して Step 2.5 へ進む。
（途中でユーザーに確認を求めると全自動パイプラインが途切れるため。利用者の no-prompt / 最大2プロンプト方針と整合）

**対話モード（ユーザーが「選ばせて」「候補を見せて」「確認させて」等と明示した場合のみ）**:
...
（明示的な対話モード指定があったときのみ）ユーザーの指示を待つ。
**既定＝自動モードではこの確認を行わず全件確定**し、止まらず Step 2.5 へ進む。
